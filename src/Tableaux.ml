...
let delta_rule : quantifier_rule = fun sfs sf ->
  ...
  and name_f v f =
    let rec name0 i =
      if elem (subst v (n (name i)) f, false) sfs
      then name0 (i + 1)
      else name i in
    name0 0 in
  match sf with
...
let apply_quant_rule : quantifier_rule -> signed list -> signed list =
  fun rule sfs -> let rec apply_quant_rule0 rule0 (sfs0, sfs1) =
                    ...
                    | (sf2 :: sfs2, _) ->
                      apply_quant_rule0 rule0
                        (sfs2,
                         append (rule0 (sf2 :: append sfs2 sfs1) sf2) sfs1) in
...
let rec contradictory : signed list -> bool = fun sfs ->
  match sfs with
    [] -> false
...
let loop : tableau -> tableau =
  concat_map
    (fun maybesfs -> match maybesfs with
         None -> [None]
       | Some sfs ->
         concat_map
           (fun newfs ->
