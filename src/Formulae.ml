...
type formula = Pred of int * term list
             | Top
             | Bot
...
let fv_term : term -> var list = fun t ->
  match t with
    V v -> [v]
  | N n -> []

let rec fv_formula : formula -> var list = fun f ->
  match f with
...
let names_term : term -> name list = fun t ->
  match t with
    V v -> []
  | N n -> [n]

let rec names_formula : formula -> name list = fun f ->
  match f with
